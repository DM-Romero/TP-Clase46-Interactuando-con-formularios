...
$('title').addEventListener('blur', (e)=>{
    $('info-title').innerHTML = "";
    if (e.target.value.length < 2) $('error-title').innerHTML = "Tan corto es el titulo de la pelieeecula?!?!!>:( $&*[#¡?"
    if (!e.target.value) $('error-title').innerHTML = "Te dije que pongas el nombre de la pelieee!!!>:("

})

$('rating').addEventListener('focus', ()=>{
    $('info-rating').innerHTML = "Decime cual ,cual ,cual es el rating"
    setTimeout(()=>{
...
$('rating').addEventListener('blur', (e)=>{
    ...
    if (e.target.value > 10) $('error-rating').innerHTML = "Paráaaaaa! (°o°)"

    if (!e.target.value) $('error-rating').innerHTML = "No averiguaste el ratingggg?!!!>:("
})
